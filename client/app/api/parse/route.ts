...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!query)
    return Response.json({ intent: EMPTY_INTENT, semanticQuery: "", keywords: [] });
  const { intent, semanticQuery, keywords, degraded } = await parseIntentLLM(query);
  if (degraded) {
    // LLM 파싱 실패(키 미설정·오류) → 502로 알려 parse-query-remote가 규칙 파서로 폴백하게 한다.
    return Response.json({ intent, semanticQuery, keywords }, { status: 502 });
  }
  return Response.json({ intent, semanticQuery, keywords });
}
