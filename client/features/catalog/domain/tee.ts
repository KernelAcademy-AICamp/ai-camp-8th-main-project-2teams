// 도메인 엔티티 — 프레임워크/데이터소스 독립. 순수 타입과 상수만.

export type ColorKey =
  | "흰"
  | "검정"
  | "회색"
  | "네이비"
  | "노랑"
  | "빨강"
  | "파랑"
  | "초록"
  | "주황"
  | "분홍"
  | "보라";

export const COLOR_HEX: Record<ColorKey, string> = {
  흰: "#ffffff",
  검정: "#1a1b1f",
  회색: "#9ca3af",
  네이비: "#1e293b",
  노랑: "#f5c518",
  빨강: "#ef4444",
  파랑: "#3b82f6",
  초록: "#22c55e",
  주황: "#ff7a2f",
  분홍: "#ff7aa2",
  보라: "#8b5cf6",
};

export type PrintPosition = "앞" | "뒤" | "양면";
export type GraphicType = "레터링" | "캐릭터" | "로고" | "패턴" | "그래픽";
export type Fit = "오버" | "레귤러" | "슬림";
export type Material = "면" | "폴리" | "기능성";
export type Gender = "male" | "female" | "unisex";

// 런타임 검증용 상수 — LLM 출력이 허용값인지 확인할 때 사용 (타입은 컴파일 시 소거되므로 별도 필요).
export const COLOR_KEYS = Object.keys(COLOR_HEX) as ColorKey[];
export const PRINT_POSITIONS: readonly PrintPosition[] = ["앞", "뒤", "양면"];
export const GRAPHIC_TYPES: readonly GraphicType[] = [
  "레터링",
  "캐릭터",
  "로고",
  "패턴",
  "그래픽",
];
export const FITS: readonly Fit[] = ["오버", "레귤러", "슬림"];
export const FUNCTIONALS: readonly string[] = ["냉감", "통풍", "신축", "흡습속건"];
export const MATERIALS: readonly Material[] = ["면", "폴리", "기능성"];
export const GENDERS: readonly Gender[] = ["male", "female", "unisex"];
export const GENDER_LABEL: Record<Gender, string> = {
  male: "남성",
  female: "여성",
  unisex: "공용",
};

// ── 리뷰 정보태그(review_tags) 닫힌 집합 ──
// 정본: backend/review_analysis/정보태그_사전.md (resolve ALLOWED와 동기화 유지).
// POSITIVE = 검색으로 "원하는" 태그(매칭 대상). NEGATIVE = 결함/기피 태그(제외 필터 전용).
export const REVIEW_TAGS_POSITIVE: readonly string[] = [
  "오버핏",
  "루즈핏",
  "슬림핏",
  "넉넉핏",
  "박시핏",
  "넥라인넉넉",
  "넥라인좁음",
  "체형보정",
  "활동성좋음",
  "엉덩이커버기장",
  "부드러움",
  "탄탄함",
  "재질좋음",
  "면느낌",
  "안달라붙음",
  "구김적음",
  "털안붙음",
  "얇음",
  "두께적당",
  "도톰함",
  "가벼움",
  "신축성좋음",
  "냉감",
  "통풍",
  "흡습속건",
  "여름용",
  "간절기용",
  "겨울이너",
  "따뜻함",
  "화면색상일치",
  "선명한발색",
  "컬러다양",
  "색상예쁨",
  "목안늘어남",
  "세탁후변형없음",
  "건조기OK",
  "마감깔끔",
  "프린팅튼튼",
  "보풀안생김",
  "비침없음",
  "이염없음",
  "초크안묻음",
  "시스루",
  "피그먼트워싱",
  "옆선무봉제",
  "디자인귀여움",
  "클라이밍",
  "운동복",
  "러닝",
  "데일리",
  "이너·레이어드",
  "홈웨어·잠옷",
  "작업복",
  "단체티·유니폼",
  "크루티",
  "선물용",
  "커플티",
  "가족티",
  "여행·휴가",
  "골프",
  "등산·산책",
  "복싱",
  "테니스",
  "필라테스",
  "아동·학생용",
  "행사·공연",
  "각인서비스",
];
export const REVIEW_TAGS_NEGATIVE: readonly string[] = [
  "무거움",
  "한여름엔더움",
  "까슬함",
  "땀나면달라붙음",
  "구김생김",
  "택따가움",
  "먼지잘묻음",
  "털잘붙음",
  "스판없음",
  "목늘어남",
  "세탁후줄어듦",
  "건조기주의",
  "봉제불량",
  "프린팅벗겨짐",
  "보풀생김",
  "비침있음",
  "물빠짐",
  "화면과색상다름",
  "홀드달라붙음",
  "밑단말림",
  "땀안마름",
  "단독세탁필요",
];

export interface Tee {
  id: string;
  name: string;
  brand: string;
  brandCanonical?: string; // 사전 매칭된 통합 브랜드(검색축). 없으면 미상.
  gender: Gender; // 제목 규칙 판정. DB NOT NULL default라 항상 존재(신호 없으면 unisex).
  price: number;
  mall: string;
  link: string; // 상품 페이지(몰) URL — 구매 진입(outbound) 대상
  image?: string; // 실상품 사진 URL(Supabase image_url). 없으면 합성 swatch로 폴백.
  // ── 추출 속성: 추출 파이프라인 전이라 NULL일 수 있음 → optional. 값 있을 때만 UI 표기. ──
  baseColor?: ColorKey;
  printColor?: ColorKey;
  printPosition?: PrintPosition;
  graphicType?: GraphicType;
  fit?: Fit;
  material?: Material;
  functional: string[]; // 냉감 · 통풍 · 신축 · 흡습속건 (없으면 [])
  reviewTags: string[]; // 리뷰에서 추출한 정보태그(긍정+부정 혼재, 없으면 [])
  sizes: string[]; // ["S","M","L","XL"] 또는 ["프리"] (없으면 [])
}
