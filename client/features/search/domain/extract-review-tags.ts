...
const RULES: Rule[] = [
  // ── 프린팅/원단 "튼튼" 구분 ──
  {
    re: /(프린팅|인쇄|프린트|그래픽|나염|프린)\s*(이|은|는|가)?.{0,5}(안\s*벗겨|튼튼|안\s*갈라|오래|안\s*지워|안\s*떨어|변하지\s*않|안\s*변하|변형\s*없|안\s*변색|그대로|안\s*지워지|색\s*안\s*빠)|(안\s*벗겨|안\s*갈라|변하지\s*않).{0,5}(프린팅|인쇄|프린트|그래픽|나염)/,
    tag: "프린팅튼튼",
    bucket: "review",
  },
  {
